...
import { EventFormSkeleton } from "@/components/ui/Skeleton";
...
function EditEventPage() {
	const { eventId } = useParams({ from: "/events_/$eventId" });
	...
	useEffect(() => {
		if (event) {
			reset({
				...
				event_date: dayjs(event.eventDate).format("YYYY-MM-DDTHH:mm"),
				location: event.location || "",
				link: event.link || "",
				description: event.description || "",
				limit_participants: event.limitParticipants?.toString() || "",
